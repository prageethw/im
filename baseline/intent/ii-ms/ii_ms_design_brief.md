...
Intent Intelligence MS, referred to as II MS, owns semantic interpretation, Knowledge Plane-backed validation and required pre-resolution validation, and semantic and service-ready preparation for admitted runtime intents.
...
| External TMF lifecycle and status projection | IC MS |
| External event subscriptions | IC MS or ID MS depending on resource |
| Downstream optimisation selection | Optimiser platform |
| Optimiser backend and solver target | `t7-gurobi-optimiser` where selected by KP or optimiser configuration |
| Assurance, apply, and runtime truth | IA MS |
| Callback ingestion | ICB MS |
| Raw network apply and change execution | Change execution layer and network orchestrator |
| KP config CRUD and governance | Knowledge Plane operating model |
...
II MS expects the admitted expression to preserve the canonical ID and IC semantic grouping:
...
| Idempotency | Deduplicate by CloudEvents `ce-id` and event id and `intentId` |
...
| Audit | Record semantic and KP decision trail where required |
...
For in-flight updates or cancellation, II MS must check the current runtime intent version and state before submitting optimisation, before consuming an optimiser outcome, and before publishing any milestone event. A stale `IntentValidatedEvent` or `OptimisationStatusChangeEvent` for a superseded or cancelled runtime version must be ignored or recorded for audit, and must not produce a new `IntentNetworkReadyEvent`.
...
| KP area | II MS use |
|---|---|
| `locationBasedServices` | Resolve location and service availability and display labels |
| `capabilityStatus` | Decide available, unknown, or unavailable capability |
| `benchmarks` | Understand service capability baseline and service and location target thresholds |
...
| `expressionMapping` | Resolve aliases and human expressions where applicable |
...
II MS must apply a configured KP freshness policy before using KP facts for safety-critical resolution. Where KP data is stale, missing, or cannot be refreshed within the configured freshness window, II MS must fail closed for the affected runtime version and use an intent-domain reason such as `KNOWLEDGE_LOOKUP_ERROR` rather than emitting an unsafe `IntentResolvedEvent` or `IntentNetworkReadyEvent`. In-flight intents must re-check the current KP snapshot and version before service-ready packaging when optimisation or callback wait time has exceeded the configured freshness threshold.
...
- carries selected apply and change-execution details under `serviceConfiguration.orchestratorConfiguration`
- carries assurance and monitoring details under `serviceConfiguration.observerConfiguration`
...
`IntentNetworkReadyEvent.serviceConfiguration.orchestratorConfiguration.resources[]` contains only the selected apply configuration chosen after optimisation and selection, not the full applicable candidate set.
...
II MS emits `IntentRejectedEvent` when the admitted intent is syntactically valid but cannot be semantic, policy, or capability resolved. II MS reason codes are intent-domain reason codes. They must not leak downstream implementation or selection vocabulary.
...
| `IntentResolvedEvent.lifecycleStatus = InProgress` | `Intent.lifecycleStatus = InProgress` while downstream fulfilment and preparation continues |
| `IntentNetworkReadyEvent.lifecycleStatus = InProgress` | `Intent.lifecycleStatus = InProgress` while apply and assurance continues |
...
Any future operational or admin endpoint must be platform-internal only and must not be presented as a TMF921 resource API.
...
| Concern | Baseline |
|---|---|
| DB type | Managed PostgreSQL or PostgreSQL-compatible RDBMS |
...
| `intent_resolution_idempotency` | Deduplication and idempotency tracking for consumed events |
...
| `intent_resolution_audit` | Semantic and KP lookup decisions and rejection reasons |
...
|---|---|
| II MS DB unavailable | Hard fail processing; do not acknowledge event until retry and dead-letter policy applies |
...
- restrict operational probes to the internal runtime and network plane
...
- no raw KP credentials or endpoint details in logs and events
...
- KP lookup latency and error metrics
- idempotency duplicate counters
- outbox pending and publish failure metrics
