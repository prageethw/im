...
| **Rule** | **Baseline** |
|---|---|
| Draft is authoring only | `Draft` Intents are saved for authoring. They are not admitted, optimised, assured, sent downstream, or used to drive `activeVersion`. |
| Submitted admission requires explicit definition reference | Submitted admission requires both `intentSpecification.id` and `expression.iri`. |
| IC MS validates contract shape only | IC MS performs schema and request-shape validation against the selected `ACTIVE` `IntentSpecification`. Semantic, policy, feasibility, optimisation, and assurance decisions are downstream concerns. |
| IC MS projects lifecycle | IC MS owns the external lifecycle/status projection but does not own runtime truth. Runtime truth comes from downstream outcomes, especially II MS and IA MS. |
| Internal and external event paths are separate | Internal state/progress events use Kafka through the internal event outbox. External TMF notifications use REST webhook callbacks through the webhook delivery outbox. |
| DELETE means termination | `DELETE /intent/{id}` is treated as runtime termination and retention, not physical deletion. |
| GET returns current projection | `GET /intent/{id}` and `GET /intent` return current projected Intent state, not the full internal version aggregate. |

### Request vs persisted response quick reference:

| **Case** | **Consumer request** | **Persisted or returned response** |
|---|---|---|
| Draft create request | `POST /intent` with `submit: false`. `intentSpecification.id` and `expression.iri` are optional Draft content only. | Draft authoring record with `lifecycleStatus: Draft`, server-generated `id`, ETag, links, and no permanent runtime version. |
| Draft create response | Not applicable; response is generated by IC MS. | Persisted Draft projection. Draft revision is represented by ETag. Draft does not drive `activeVersion`. |
| Submitted admission request | `POST /intent` with `submit: true` or omitted `submit`, carrying mandatory `intentSpecification.id` and mandatory `expression.iri`. | Admitted runtime version with `lifecycleStatus: Acknowledged`; IC MS emits `IntentValidatedEvent`. |
| Persisted admitted response | Not applicable; response is generated by IC MS. | Persisted external Intent projection with runtime `version`, lifecycle/status projection, server-resolved `isBundle`, and selected `IntentSpecification` reference. |
| GET projection | `GET /intent/{id}` or `GET /intent`. | Current projected Intent state only. Historical versions, `Standby`, `Retired`, rollback candidates, and full version aggregates remain internal unless exposed through `IntentReport` or a documented extension. |

> **Warning:** Do not implement IRI-only admission, `specKey`-only admission, name-only admission, or payload-shape inference. Submitted admission is governed by explicit `intentSpecification.id` plus the `expression.iri` consistency check.
...
### Expression IRI mismatch:

The following request is rejected because the submitted `expression.iri` does not match the selected `IntentSpecification.expressionSpecification.iri` for `ispec-hss-001` version `1.20`.
...
```json
{
  ...
  "expression": {
    "@type": "JsonLdExpression",
    "@baseType": "Expression",
    "iri": "https://mycsp.com.au/tio/unsupported-intent/v1.0",
    "expressionValue": {
      "context": {
        "targets": {},
        "constraints": {},
...
{
  "code": "VALIDATION_FAILED",
  "reason": "INTENT_EXPRESSION_IRI_MISMATCH",
  "message": "Runtime expression.iri does not match the selected IntentSpecification expressionSpecification.iri.",
  "status": 422,
  "referenceError": "https://mycsp.com.au/errors/VALIDATION_FAILED",
  "@type": "Error"
}
