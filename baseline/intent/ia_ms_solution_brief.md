...
| Field / area | IA MS usage |
|---|---|
| `body.eventType` | Confirms callback event type where supplied. |
| `body.eventVersion` | Callback event contract version. |
| `body.eventTime` | Relay event time. |
| `body.correlationId` | Correlation across the workflow. |
...
| `body.sourceState.state` | Raw source/orchestrator state value to map. |
| `body.sourceState.reason` | Optional raw reason to support IA mapping/audit. |

Do not use `orchestratorState`, `orchestratorSource`, or `orchestratorTimestamp` as baseline callback field names.
...
| `orchestratorState` / `orchestratorSource` / `orchestratorTimestamp` | Replaced by `sourceState`, `callbackSource`, and `callbackTimestamp`. |
