...
Reactivation of a SUSPENDED subscription is operationally governed in the baseline unless an explicit subscription update or reactivation endpoint is introduced later. Reactivation of a `SUSPENDED` subscription is operationally governed in the baseline unless an explicit subscription update or reactivation endpoint is introduced later. `@type` is `EventSubscription`. `_links` exposes only valid subscription actions for the current caller.
