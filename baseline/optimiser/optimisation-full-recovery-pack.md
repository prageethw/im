...
4. OEX APIs route through OSB MS (OEX APIs).
5. OSB MS routes.
6. OSB MS calls NGW.
7. NGW calls OC MS.
...
OEX / OGW / OEX APIs / OSB MS (OEX APIs) / OSB MS are experience-layer/private integration components and do not need to be TMF-compliant.
...
2. OEX UI calls OGW.
3. OGW invokes OSB MS (OEX APIs) using mTLS and User Context JWT.
4. OSB MS validates user context and exposes the cancellation action only when allowed.
5. OSB MS calls NGW using mTLS and OAuth2 system-to-system.
6. NGW routes POST /optimisation/{id}/cancellation with If-Match to OC MS.
7. OC MS validates ETag and lifecycleStatus.
8. OC MS updates lifecycleStatus to CANCELLING in OC MS DB.
9. OC MS writes OptimisationRequestedEvent with instruction = CANCEL to OC MS Outbox.
10. OC MS Outbox relay publishes the cancellation instruction to Kafka.
11. Python/Gurobi Worker consumes or observes the CANCEL instruction.
12. Worker stops, cancels, or ignores work where safely possible.
13. OC MS Inbox/operational handling later projects CANCELLED when cancellation is confirmed or safely resolved.
14. User polls GET /optimisation/{id} through OEX UI -> OGW -> OSB MS (OEX APIs) -> NGW -> OC MS.
```


Detailed retrial flow:

```text
1. User initiates retrial from OEX UI.
2. OEX UI calls OGW.
3. OGW invokes OSB MS (OEX APIs) using mTLS and User Context JWT.
4. OSB MS validates user context and exposes the retrial action only when allowed.
5. OSB MS calls NGW using mTLS and OAuth2 system-to-system.
6. NGW routes POST /optimisation/{id}/retrial with If-Match to OC MS.
7. OC MS validates original Optimisation lifecycleStatus = FAILED.
8. OC MS creates a new runtime Optimisation in OC MS DB.
9. The new Optimisation links to the failed Optimisation using retrialOf.
10. The new Optimisation starts with lifecycleStatus = ACKNOWLEDGED.
11. OC MS writes OptimisationRequestedEvent with instruction = EXECUTE to OC MS Outbox.
12. OC MS Outbox relay publishes the execute instruction to Kafka.
13. Python/Gurobi Worker consumes the event and invokes Gurobi Optimizer.
14. User polls GET /optimisation/{id} for the new Optimisation through OEX UI -> OGW -> OSB MS (OEX APIs) -> NGW -> OC MS.
```
